    ...
    for root, director, files in os.walk(folder):
        for file in files:
            if i < num:
                ...
                extend_x = 0.2 + 0.1 * np.random.uniform(-1, 1, 1)
                extend_y = 0.5 + 0.1 * np.random.uniform(-1, 1, 1)

                right_x = rectangle[1] + int(extend_x*plate_w)
                if right_x >= shape[0]:
                    right_x = 1159
                left_x = rectangle[0] - int((0.4 - extend_x)*plate_w)
                if left_x < 0:
                    left_x = 0
                top_y = rectangle[2] - int(extend_y*plate_h)
                if top_y < 0:
                    top_y = 0
                bottom_y = rectangle[3] + int((1 - extend_y) * plate_h)
                if bottom_y >= shape[0]:
                    bottom_y = 719
                # print([top_y, bottom_y, left_x, right_x])

                extend_img = img_gray[top_y:bottom_y, left_x:right_x]
                extend_img = transform.resize(extend_img, (120, 300))
                ...
                data.append(vec)
                print([plate_norm_ltx, plate_norm_lty, plate_norm_lbx, plate_norm_lby,
...
def get_model(path):
    ...
    model = Sequential()
    model.add(Convolution2D(1, 7, 7, border_mode='valid', input_shape=(120, 300, 1), subsample=(2, 2)))
...
def train_model():
    model = get_model('null')
    t_data, t_labels = gen_train_set('D:/dataset/ccpd_select2/', 3000)
...
def test_model():
    ...
    print(t_labels)
    print('\n')
    points = model.predict(t_data)
    print(points)

    model = get_model('D:/dataset/regression.m')
    t_data, t_labels = gen_train_set('D:/dataset/ccpd_select2/', 1)
    print(t_labels)
    print('\n')
    points = model.predict(t_data)
    print(points)
